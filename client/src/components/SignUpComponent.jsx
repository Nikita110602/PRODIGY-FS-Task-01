  ...
  return (
    <>
        <form>
            ...
            <h2 className=' text-2xl mt-1 mb-1 font-bold mx-36'>Sign Up</h2>
                <input type="text" name='Fullname' className='h-10 p-5 rounded-lg w-64 mx-5 my-4 bg-[#1e1b4b]' value={data.Fullname} placeholder='Fullname' onChange={(event)=>{setData({...data,Fullname:event.target.value})}}/> <br />
                <input type="date" name='DOB' className='h-10 p-5 rounded-lg w-64 mx-5 my-4 bg-[#1e1b4b]' value={data.Dob} placeholder='Date of Birth' onChange={(event)=>{setData({...data,Dob:event.target.value})}}/> <br />
                ...
                <p className='mb-16 bg-[#1e1b4b]'>Already User? <Link to='/'>Sign In</Link></p>
